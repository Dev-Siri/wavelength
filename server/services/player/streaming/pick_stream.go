...
func PickAndGenerateStreamURL(ctx *fiber.Ctx, streamPartName string, clientDetails *security.TokenClientDetails) (string, error) {
	if !clientDetails.IsHifi {
		...
		reader, err := hifiAutoStreamingHandler(ctx, streamPartName, clientDetails.ISRC, clientDetails.VideoID)
		if err != nil {
			return "", err
		}
