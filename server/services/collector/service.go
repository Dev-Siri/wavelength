package main

import (
	"log"
	"net"

	"github.com/Dev-Siri/wavelength/server/proto/collectorpb"
	collector_rpcs "github.com/Dev-Siri/wavelength/server/services/collector/rpcs"
	"github.com/Dev-Siri/wavelength/server/shared/clients"
	shared_db "github.com/Dev-Siri/wavelength/server/shared/db"
	shared_env "github.com/Dev-Siri/wavelength/server/shared/env"
	"github.com/Dev-Siri/wavelength/server/shared/logging"
	"go.uber.org/zap"
	"google.golang.org/grpc"
)

func main() {
	if err := logging.InitLogger(); err != nil {
		log.Fatalf("Failed to initialize logger: %s", err.Error())
	}

	if err := shared_env.InitEnv(); err != nil {
		logging.Logger.Error("Failed to initialize environment variables.", zap.Error(err))
	}

	if err := shared_db.ConnectStreamDatabase(); err != nil {
		logging.Logger.Error("Failed to initialize stream Postgres connection.", zap.Error(err))
	}

	if shared_db.StreamDatabase != nil {
		defer func() {
			if err := shared_db.StreamDatabase.Close(); err != nil {
				logging.Logger.Fatal("Failed to close stream database connection.", zap.Error(err))
			}
		}()
	}

	if err := shared_db.InitObjectStorage(); err != nil {
		logging.Logger.Error("Failed to initialize GCS Object Store.", zap.Error(err))
	}

	if err := clients.InitYtScraperClient(); err != nil {
		logging.Logger.Error("YtScraper-service client failed to initialize.", zap.Error(err))
	}

	port := shared_env.GetPORT()
	addr := ":" + port

	listener, err := net.Listen("tcp", addr)
	if err != nil {
		logging.Logger.Error("TCP Listener failed to listen on provided address.", zap.String("addr", addr), zap.Error(err))
	}

	grpcServer := grpc.NewServer(
		grpc.UnaryInterceptor(logging.GrpcLoggingInterceptor),
	)
	collectorService := collector_rpcs.NewCollectorService()
	collectorpb.RegisterCollectorServiceServer(grpcServer, collectorService)

	logging.Logger.Info("CollectorService listening on "+addr, zap.String("port", port))
	if err := grpcServer.Serve(listener); err != nil {
		logging.Logger.Error("CollectorService launch failed.", zap.Error(err))
	}
}
