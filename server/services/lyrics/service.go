...
import (
	...
	lyrics_rpcs "github.com/Dev-Siri/wavelength/server/services/lyrics/rpcs"
...
func main() {
	...
	if err := clients.InitYtScraperClient(); err != nil {
		logging.Logger.Error("Initializing YTScraper-service failed.", zap.Error(err))
	}
	...
	lyricService := lyrics_rpcs.NewLyricService()
	lyricspb.RegisterLyricServiceServer(grpcServer, lyricService)

	logging.Logger.Info("LyricService listening on "+addr, zap.String("port", port))
	if err := grpcServer.Serve(listener); err != nil {
		logging.Logger.Error("LyricService launch failed.", zap.Error(err))
	}
}
