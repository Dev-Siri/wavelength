<script lang="ts">
  ...
  function removeMusicFromQueue(
    ...
    musicQueueStore.musicPlaylistContext = musicQueueStore.musicPlaylistContext.filter(
...
>
  ...
  <div class="flex flex-col items-start ml-2 justify-center">
    <p class="text-sm">
      {musicQueueItem.title.length > 15
        ? `${musicQueueItem.title.slice(0, 15)}...`
