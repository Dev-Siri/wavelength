...
import "package:wavelength/widgets/music_player_preview/music_player_preview_body.dart";
    ...
    return BlocBuilder<MusicPlayerTrackBloc, MusicPlayerTrackState>(
      builder: (context, state) {
        return GestureDetector(
          onTap: () {
            if (state is! MusicPlayerTrackPlayingNowState) return;
            ...
            showModalBottomSheet(
              ...
              builder: (_) => const PlayingNowScreen(),
            );
          },
          ...
          onLongPressMoveUpdate: (details) {
            if (state is! MusicPlayerTrackPlayingNowState) return;
            _handleMusicPlayerTrackProgressUpdate(details);
          },
          onLongPressEnd: (details) {
            if (state is! MusicPlayerTrackPlayingNowState) return;
            _handleMusicPlayerTrackSeekToUpdate(details);
          },
          child: BlocBuilder<CoverEffectColorsBloc, CoverEffectColorsState>(
            builder: (context, colorState) {
              return Container(
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(15),
                  color: colorState is CoverEffectColorsSuccessState
                      ? Color.fromRGBO(
                          colorState.colors[0].r,
                          colorState.colors[0].g,
                          colorState.colors[0].b,
                          0.6,
                        )
