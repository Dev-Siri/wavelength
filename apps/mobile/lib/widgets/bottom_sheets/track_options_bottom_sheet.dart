...
class _TrackOptionsBottomSheetState extends State<TrackOptionsBottomSheet>
  ...
  Future<void> _fetchTrackAlreadyDownloaded() async {
    ...
    final exists = box.containsKey(widget.track.videoId);
  ...
  void _addToQueue() {
    context.read<WavelengthAudioHandler>().addToQueue(
      QueueableMusic(
        videoId: widget.track.videoId,
        title: widget.track.title,
        thumbnail: widget.track.thumbnail,
    ...
    showToast(context, "Added ${widget.track.title} to queue.", ToastType.info);
